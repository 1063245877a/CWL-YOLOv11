...
__all__ = ['DyHeadBlock', 'DyHeadBlockWithDCNV3', 'Fusion', 'C3k2_Faster', 'C3k2_ODConv', 'Partial_conv3', 'C3k2_Faster_EMA', 'C3k2_DBB',
           ...
           'A2C2f_EDFFN', 'C3k2_EBlock', 'C3k2_DBlock', 'C3k2_FDConv', 'C3k2_MambaOut_FDConv', 'C3k2_PFDConv', 'C3k2_PFDConv', 'C3k2_FasterFD', 'C3k2_DSAN', 'C3k2_MambaOut_DSA', 'C3k2_DSA', 'C3k2_DSAN_EDFFN', 'C3k2_RMB', 'C3k2_MLCAGLU'
           ]
...
# modules_hybrid.py
# PyTorch modules: MLCA (light), ConvolutionalGLU, Faster_Block_CGLU (adapted),
# and C3k2_MLCAGLU that fuses MLCA and Faster-CGLU branches.
# Place in your project and import where you define model blocks.

# -------------------------
# Small utility blocks
# -------------------------
def autopad(k, p=None):  # padding helper
    if p is None:
        p = k // 2
    return p

class Conv(nn.Module):
    """Simple conv + BN + SiLU (like Ultralytics)"""
...
# -------------------------
# MLCA: a compact local+channel attention module
# (This is a practical, lightweight design — you can swap with your implementation)
# -------------------------
class MLCA(nn.Module):
    """
    Minimal Local-Channel Attention (MLCA) block:
    - depthwise conv for local mixing
    - channel squeeze-excitation
    - optional pointwise to restore channels
    """
    def __init__(self, channels, reduction=4, dw_k=3):
        super().__init__()
        hidden = max(8, channels // reduction)
        self.dw = nn.Conv2d(channels, channels, kernel_size=dw_k, padding=dw_k//2, groups=channels, bias=False)
...
# -------------------------
# ConvolutionalGLU (from your snippet, kept compatible)
# -------------------------
...
# -------------------------
# Faster block wrapper (from your snippet, adapted)
# -------------------------
class Faster_Block_CGLU(nn.Module):
    def __init__(self,
        ...
        # a placeholder spatial mixing. Replace Partial_conv3 with your implementation if available.
        self.spatial_mixing = nn.Conv2d(dim, dim, kernel_size=3, padding=1, groups=1, bias=False)
        ...
        if layer_scale_init_value > 0:
            ...
            self._forward = self.forward_layer_scale
        else:
...
# Minimal DropPath implementation
...
# -------------------------
# Bottleneck_MLCA: reuse simple bottleneck with MLCA inside
# -------------------------
...
class C3k2_MLCAGLU(nn.Module):
    def __init__(self, c1, c2, shortcut=True, g=2, e=0.5):
        super().__init__()
        hidden = int(c2 * e)
        ...
        self.m = nn.Sequential(
            ...
            Conv(hidden, hidden, k=1)
        )
        self.shortcut = shortcut and c1 == c2

        # channel split
        ratio = 0.25
        mlca_c = max(1, int(hidden * ratio))

        # ----------- FIX: ensure valid group setting -----------
        g_eff = max(1, min(g, mlca_c))
        if mlca_c % g_eff != 0:
            # choose nearest valid group divisor
            for div in [8, 4, 2, 1]:
                if mlca_c % div == 0:
                    g_eff = div
                    break
        ...
        self.mlca = nn.Sequential(
            nn.Conv2d(mlca_c, mlca_c, 1, groups=g_eff),
            nn.ReLU(inplace=True),
            nn.Conv2d(mlca_c, mlca_c, 1, groups=g_eff),
            nn.Sigmoid()
        )
    ...
    def forward(self, x):
        ...
        # concat
        y = torch.cat([y_att, y_main], dim=1)
...
# -------------------------
# Note: If you integrate into Ultralytics YOLO repo:
# - put this file under models/ or models/common.py
# - import C3k2_MLCAGLU and register as available module name in the factory
# - update model yaml replacing occurrences of 'C3k2' with 'C3k2_MLCAGLU'
# -------------------------
